...
function handleSharpMindSelection_beta(sharpMindInstance) {
  if (!sharpMindInstance) return;

  const mainPath = pathDisplaySelect ? pathDisplaySelect.value || "" : "";
  if (!mainPath) {
  ...
  // find skills on main path, not above Scholar tier, and not already boosted for that skill
  const alreadyBoosted = new Set(
    (sharpMindAssignments_beta || []).map(
      (a) => `${a.targetPath}::${a.targetName}`
    )
  );
...
" +
        "Remember, it cannot be applied to the same skill more than once,
" +
  ...
  sharpMindAssignments_beta.push({
    sharpPath: sharpMindInstance.path,
    sharpName: sharpMindInstance.name,
    sharpTier: parseInt(sharpMindInstance.tier || 0, 10) || 0,
  ...
  alert(
    `Sharp Mind applied (beta): ${target.name} (Tier ${target.tier || 0}) now has +1 use per day.`
  );
}

function getSharpMindNotesForDisplay_beta(skill) {
  if (!skill || !Array.isArray(sharpMindAssignments_beta)) return "";
  if (!sharpMindAssignments_beta.length) return "";

  const path = skill.path || "";
  const name = skill.name || "";
  const tier = parseInt(skill.tier || 0, 10) || 0;
  ...
  const asSource = sharpMindAssignments_beta.filter(
    (a) =>
      a.sharpPath === path &&
      a.sharpName === name &&
      (parseInt(a.sharpTier || 0, 10) || 0) === tier
  );
  ...
  if (asTarget.length) {
    const tiers = Array.from(
      new Set(
        asTarget.map((a) => parseInt(a.sharpTier || 0, 10) || 0)
      )
    ).sort((a, b) => a - b);
    const tierLabel = tiers.map((t) => `Scholar Tier ${t}`).join(", ");
    const bonus = asTarget.length; // should normally be 1 per rules
    parts.push(
  ...
  if (asSource.length) {
    ...
    parts.push(
      `This Sharp Mind (Scholar Tier ${tier}) is applied to: ${targets.join(
        ", "
      )}.`
    );
  }
...
function addSelectedSkill() {
  ...
  // Sharp Mind beta hook: attach to a Main Path skill
  if (path === "Scholar" && name === "Sharp Mind") {
    try {
      handleSharpMindSelection_beta(candidateRecord);
    } catch (e) {
